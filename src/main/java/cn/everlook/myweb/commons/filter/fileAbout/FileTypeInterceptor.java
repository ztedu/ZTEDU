...
public class FileTypeInterceptor extends HandlerInterceptorAdapter {
    ...
    if (request instanceof MultipartHttpServletRequest) {
      ...
      while (iterator.hasNext()) {
        ...
        if (!checkFile(filename)) {
          ...
          request.getRequestDispatcher("/themes/lf/pc/app/common/error/fileTypeNoteSupport.jsp").forward(request, response);
          flag = false;
        }
      }
    }
  ...
  private boolean checkFile(String fileName) {
    //设置允许上传文件类型
    String suffixList =  SystemConfig.get("sys.upload.type");
    // 获取文件后缀
