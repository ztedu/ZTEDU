        ...
        <div class="layui-card-body mini-bar">
          <blockquote class="layui-elem-quote layui-text">
            ...
            <div class="layui-input-inline mr0">
              <input id="key" class="layui-input" type="search" placeholder="输入部门名称" autocomplete="off"/>
            </div>
            <br/>
          </blockquote>
    ...
    <div class="layui-col-sm12 layui-col-md8 layui-col-lg9" id="rightTable">
      <div class="layui-card">
        <div class="layui-card-body">
            ...
            <blockquote class="layui-elem-quote layui-text">
              ...
              <div class="layui-form-item">
                <div class="layui-inline">
                  ...
                  <div class="layui-input-inline mr0">
                    <input type="text" name="name" id="name" autocomplete="off" class="layui-input" placeholder="部门名称"/>
                  </div>
                </div>
                <div class="layui-inline">
                  <label class="layui-form-label">部门地址:</label>
                  <div class="layui-input-inline mr0">
                    <input type="text" name="loc" autocomplete="off" class="layui-input" placeholder="部门地址"/>
                  </div>
                </div>
                <div class="layui-inline">
                  <label class="layui-form-label">部门备注:</label>
                  <div class="layui-input-inline mr0">
                    <input type="text" name="remark" autocomplete="off" class="layui-input" placeholder="部门备注"/>
                  </div>
                </div>
              </div>
              <div class="layui-form-item" style="text-align: center;">
                ...
                <shiro:hasPermission name="dept:create">
                  <a id="add_dept_btn" class="layui-btn layui-btn-warm icon-btn">
          ...
          <%--表格--%>
          <table class="layui-table" id="deptList" lay-filter="deptList"></table>
        </div>
      </div>
    </div>
  ...
  layui.config({
    ...
    function initDeptTree() {
      $.ajax({
        ...
        dataType: "json",
        url: "<%=contextPath%>/dept/loadDeptLeftTree.do",
      ...
      cols: [[
        {
          ...
          field: 'id', title: '部门ID', minWidth: 50, width: 80, align: 'center', sort: true
        }, {
          field: 'pid', title: '父级部门ID', minWidth: 110, align: 'center', sort: true
        }, {
          field: 'name', title: '部门名称', minWidth: 100, align: 'center'
        }, {
          field: 'loc', title: '部门地址', minWidth: 100, align: 'center'
        }, {
          field: 'remark', title: '备注', minWidth: 100, width: 150, align: 'center'
        }, {
        ...
        //   }, title: '创建时间'
        // },
        {align: 'center', fixed: 'right', toolbar: '#dept-tool-bar', title: '操作', minWidth: 170}
      ]],
      toolbar: true,
    ...
    $("#reset_btn").on("click", function () {
      // zTree.getZTreeObj("ltTree").cancelSelectedNode();
      refreshTree("ltTree");
      form.render();
      table.reload('deptListTable', {
        url: '/dept/loadAllDepts.do',
      ...
      var index = layui.layer.open({
        ...
        success: function (layero, index) {
          ...
          if (addSuccess) {//添加部门成功的时候才能进入该方法刷新树和右边列表
            ...
            var parent = false;
            var spread = false;
            if (addedNewDept.parent === 1) {
              parent = true;
            }
            if (addedNewDept.spread === 1) {
              spread = true;
            }
            zTreeAdd.addNodes(parentTreeNode, {
              id: addedNewDept.id,
              pId: addedNewDept.pid,
              name: addedNewDept.name,
              isParent: parent,
              open: spread
            });
      ...
      var index = layui.layer.open({
        ...
        success: function (layero, index) {
          ...
          if (upadteNodeSuccess) {
            ...
            var updateNode = zTreeUpdate.getNodeByParam("id", dept.id);
            updateNode.name = updatedNodeDept.name;
            updateNode.pId = updatedNodeDept.pid;
            if (updatedNodeDept.parent === 1) {
              updateNode.isParent = true;
            } else {
              updateNode.isParent = false;
            }
            if (updatedNodeDept.spread === 1) {
              updateNode.open = true;
            } else {
              updateNode.open = false;
            }

            if (dept.id == 1) {//更改的节点是根节点
              // updateNode.pId = updatedNodeDept.pid;
              zTreeUpdate.updateNode(updateNode);
            } else {//更改的节点不是根节点
              if (dept.pid == updatedNodeDept.pid) {
                zTreeUpdate.updateNode(updateNode);
              } else {
                var parentTreeNode = zTreeUpdate.getNodeByParam("id", addedNodeParent.id);
                zTreeUpdate.removeNode(zTreeUpdate.getNodeByParam("id", dept.id));
        ...
        $.post("/dept/deleteDept.do", {
          ...
          if (data.code === 901) {
            ...
            layer.msg('部门删除成功', {
              ...
              var deleteNode = zTreeDelete.getNodeByParam("id", dept.id);
        ...
        layer.confirm('确定删除选中的部门吗?', {icon: 3, title: '提示信息'}, function (index) {
          ...
          $.post("/dept/batchDeleteDept.do?" + ids, function (data) {
            ...
            if (data.ret) {
              ...
              //刷新左边的树
              var zTreeDelete = zTree.getZTreeObj("ltTree");
              for (var idDept in deptIdArray) {
                var deleteNode = zTreeDelete.getNodeByParam("id", deptIdArray[idDept]);
                zTreeDelete.removeNode(deleteNode);
              }
